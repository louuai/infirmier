...
import { Users, Stethoscope, CalendarCheck, Wallet, Percent, Clock, Plus, Trash2 } from "lucide-react";

export default function AdminDashboard() {
  const [stats, setStats] = useState<any>(null);
  const [tab, setTab] = useState<"overview" | "services" | "verify" | "users">("overview");
    ...
    <div className="min-h-[calc(100vh-4rem)] bg-[#03040d] text-slate-100">
      <div className="container py-8">
        ...
        <div className="mb-4 flex flex-wrap gap-2 border-b border-white/10">
          {([["overview", "Vue globale"], ["services", "Services & tarifs"], ["verify", "Vérification"], ["users", "Utilisateurs"]] as const).map(([k, l]) => (
            ...
            {stats.popularServices?.length ? (
              <div className="space-y-2">
                {stats.popularServices.map((p: any) => (
                  <div key={p.slug} className="flex items-center justify-between border-b border-white/5 py-2 last:border-0">
                    <span className="text-slate-200">{p.slug}</span><span className="text-emerald-300">{p.count} missions</span>
                  </div>
                ))}
              </div>
        ...
        {tab === "verify" && (
          <div className="space-y-3">
            ...
            {pending.map((n) => (
              <div key={n.id} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl glass p-5">
                <div><p className="font-semibold">{n.user.firstName} {n.user.lastName}</p><p className="text-sm text-slate-400">{n.user.email} · {n.city ?? "—"} · {n.documents?.length ?? 0} doc(s)</p></div>
        ...
        {tab === "users" && (
          <div className="overflow-hidden rounded-2xl glass">
            <table className="w-full text-sm">
              ...
              <tbody>
                {users.map((u) => (
                  <tr key={u.id} className="border-b border-white/5"><td className="p-3">{u.firstName} {u.lastName}</td><td>{u.email}</td><td>{u.role}</td><td>{u.isActive ? "Actif" : "Désactivé"}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
...
function ServicesManager({ services, reload }: { services: any[]; reload: () => void }) {
  ...
  async function remove(id: string) {
    ...
    await fetch(`/api/services/${id}`, { method: "DELETE" }); reload();
  }
  async function create(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    const f = new FormData(e.currentTarget);
    const res = await fetch("/api/services", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({
      slug: String(f.get("slug")), name: String(f.get("name")), price: Number(f.get("price")), description: f.get("description") || undefined,
    }) });
